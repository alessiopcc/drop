...
namespace
{
    ...
    $test("variant/assignment", []
    {
        ...
        {
            variant <destructible, int> myvariant;
            destructible a;
            destructible b;

            myvariant = a;
            if(!myvariant.is <destructible> ())
                throw "TODO";

            lastop = 'X';
            myvariant = destructible();
            if(lastop != 'X')
                throw "Reassigning same type to a variant does call destructor.";

            myvariant = 5;
            if(lastop != 'D')
                throw "Assigning new type to a variant does not call destructor.";
